...
ros::Publisher kf_markers_array_pub;
...
void setup_publishers(ros::NodeHandle &node_handler, image_transport::ImageTransport &image_transport, std::string node_name)
{
    ...
    kf_markers_array_pub = node_handler.advertise<geometry_msgs::PoseArray>(node_name + "/kf_markers_poses", 1);
...
// More details: https://docs.ros.org/en/noetic/api/geometry_msgs/html/msg/PoseArray.html
void publish_kf_array(std::vector<Sophus::SE3f> vKFposes, ros::Time msg_time)
{
    int numKFs = vKFposes.size();
    if (numKFs == 0)
        return;
    
    geometry_msgs::PoseArray kf_markers_array;
    kf_markers_array.header.frame_id = world_frame_id;
    kf_markers_array.header.stamp = msg_time;
    
    for (int i = 0; i <= numKFs; i++)
    {
        
        geometry_msgs::Pose kf_marker_array;
        kf_marker_array.position.x = vKFposes[i].translation().x();
        kf_marker_array.position.y = vKFposes[i].translation().y();
        kf_marker_array.position.z = vKFposes[i].translation().z();
        
        
        kf_marker_array.orientation.x = vKFposes[i].unit_quaternion().coeffs().x();
        kf_marker_array.orientation.y = vKFposes[i].unit_quaternion().coeffs().y();
        kf_marker_array.orientation.z = vKFposes[i].unit_quaternion().coeffs().z();
        kf_marker_array.orientation.w = vKFposes[i].unit_quaternion().coeffs().w();

        
        kf_markers_array.poses.push_back(kf_marker_array);
    }
    
    kf_markers_array_pub.publish(kf_markers_array);
}
...
void publish_topics(ros::Time msg_time, Eigen::Vector3f Wbb)
{
    ...
    publish_kf_array(pSLAM->GetAllKeyframePoses(), msg_time);
